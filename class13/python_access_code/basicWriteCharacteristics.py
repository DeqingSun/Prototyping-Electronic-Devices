...
def main():
    ...
    try:
        ...
        device.discover([MY_SERVICE_UUID], [READ_UUID,WRITE_UUID])

        # Find the UART service and its characteristics.
        serviceObj = device.find_service(MY_SERVICE_UUID)
        readObj = serviceObj.find_characteristic(READ_UUID)
        writeObj = serviceObj.find_characteristic(WRITE_UUID)
        response = readObj.read_value();
        value=bytes2int(response)
        writeObj.write_value(int2bytes(value+1));    
        
        print hex(value)

        time.sleep(0.2) #wail til transaction finish
