...
uint8_t frameState = 0, layerState = 0;
unsigned long frameBeginTime = 0, layerBeginTime = 0;

void loop() {
  unsigned long current = millis();
  if ((current - frameBeginTime) >= 200) {
    frameBeginTime = current;
    frameState++;
    if (frameState >= 4) frameState = 0;
  }
  displayByte(pgm_read_byte(cubePattern + frameState));
}

void displayByte(uint8_t data) {
  unsigned long current = millis();
  if ((current - layerBeginTime) >= 8) {
    layerBeginTime = current;
    disableLayers();
    if (layerState == 0) {
      layerState = 1;
